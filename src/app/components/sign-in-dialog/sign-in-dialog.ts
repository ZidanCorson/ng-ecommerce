...
@Component({
  ...
  imports: [MatIconButton, MatIcon, MatDialogClose, MatFormField , MatSuffix, MatPrefix, MatInput, MatButton, ReactiveFormsModule],
  template: `
    <div class="p-8 max-w-[400px] flex flex-col">
      ...
      <form class="mt-6" [formGroup]="signInForm" (ngSubmit)="signIn()">
        ...
        <mat-form-field class="mb-6">
          <input matInput formControlName="password" [type]="passwordVisible() ? 'text' : 'password'" placeholder="Enter your password" />
          <mat-icon matPrefix>lock</mat-icon>
          <button matSuffix matIconButton type="button" (click)="passwordVisible.set(!passwordVisible())">
...
export class SignInDialog {
  ...
  signInForm = this.fb.group({  
    email: ['johntest@test.com', Validators.required],
